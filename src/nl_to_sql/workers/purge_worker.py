...
async def run_account_purge(session_factory: async_sessionmaker[AsyncSession]) -> dict[str, int]:
    ...
    for deletion in pending:
        try:
            ...
            succeeded.append(deletion.user_id)
            purged += 1
    ...
    if succeeded:
        async with session_factory() as db:
            await db.execute(
                ...
                .where(AccountDeletion.user_id.in_(succeeded))
...
async def _hard_delete_user(user_id: str, session_factory: async_sessionmaker[AsyncSession]) -> None:
